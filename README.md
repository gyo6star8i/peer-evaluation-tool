...
학생은 스마트기기로 GitHub Pages 링크에 접속해 학번, 성명, 학과, 소속 모둠, 모둠별 평가 점수를 제출합니다. 교수자는 집계 화면에서 학과별 Google Sheet 탭에 모인 응답을 불러와 모둠별 점수와 CSV를 확인합니다.

## 파일 구성

- `index.html`: 학생용 평가 입력 화면
- `admin.html`: 교수자용 집계 화면
- `config.js`: 학과, 모둠, 평가항목, Apps Script URL 설정
- `apps-script/Code.gs`: Google Sheets에 응답을 저장하고 읽어오는 Apps Script 코드

## 1. Google Sheet 수합 서버 만들기

1. 새 Google 스프레드시트를 만듭니다.
2. 시트 탭을 `작업치료학과`, `치위생학과` 두 개로 준비합니다.
3. `확장 프로그램` → `Apps Script`를 엽니다.
4. 기본 코드를 지우고 `apps-script/Code.gs` 내용을 붙여넣습니다.
5. 왼쪽 `프로젝트 설정` → `스크립트 속성`에 `ADMIN_KEY`를 추가하고 교수자 확인 키를 입력합니다.
6. 저장 후 `배포` → `새 배포`를 누릅니다.
7. 유형은 `웹 앱`으로 선택합니다.
8. 실행 권한은 `나`, 액세스 권한은 `모든 사용자`로 설정합니다.
9. 배포 후 발급되는 웹 앱 URL을 복사합니다.
...
classes: [
  {
    ...
    sheetName: "작업치료학과",
    teams: [
      { id: "ot-team-1", name: "1팀", topic: "발표 주제" }
    ]
  }
],
criteria: [
...
`sheetName`은 Google Sheet 탭 이름과 같아야 합니다. `id`는 영문/숫자/하이픈 조합을 권장합니다. 이미 응답을 받은 뒤에는 학과, 모둠, 평가항목의 `id`를 바꾸지 않는 편이 좋습니다.
...
- 선택한 학과에 따라 작업치료학과는 8팀, 치위생학과는 10팀만 표시됩니다.
- 작업치료학과 응답은 `작업치료학과` 탭, 치위생학과 응답은 `치위생학과` 탭에 저장됩니다.
